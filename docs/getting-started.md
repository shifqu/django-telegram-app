...
!!! note "**Prerequisite**"
    ...
    If you’re new to Django, start with [the official tutorial](https://docs.djangoproject.com/en/stable/intro/tutorial01/){: target="_blank" }.
...
!!! note
    It is highly recommended to work with [virtual environments](https://docs.python.org/3/library/venv.html#creating-virtual-environments){: target="_blank"}
...
Then add the following to the project's `settings.py`:
...
!!! note
    To learn how to obtain a bot token, visit the [official telegram tutorial](https://core.telegram.org/bots/tutorial#obtain-your-bot-token){: target="_blank"}
...
This creates the tables for the configured `TelegramSettings` model.

!!! note
    Most projects override the default TelegramSettings model to add their own fields as outlined [above](#3-optional-create-your-own-telegramsettings-model).
...
```text
python manage.py setwebhook <base_url>
```
...
```python title="myapp/telegrambot/commands/roll.py"
